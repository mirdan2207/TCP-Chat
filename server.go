...
func (s *server) newClient(conn net.Conn) {
	...
	c.msg("please enter your nickname: ")
	nick, err := bufio.NewReader(conn).ReadString('\n')
	if err != nil {
		log.Printf("error reading nickname: %s", err.Error())
		return
	}

	c.nick = strings.TrimSpace(nick)

	c.msg(fmt.Sprintf("wlecome, %s! you can now use the chat", c.nick))
...
func (s *server) join(c *client, args []string) {
	...
	c.msg(fmt.Sprintf("welcome to the %s", r.name))
}
